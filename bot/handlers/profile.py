...
# -------------------------------------------------------------------------
# IN-MEMORY REDIS MOCK (For Local Debug / PC Testing)
# Allows running the bot locally without spawning a real Redis instance
# -------------------------------------------------------------------------
if settings.debug:
    import asyncio
    class MemoryRedisMock:
        ...
        async def rpush(self, queue_name, val):
            ...
            # In debug mode, trigger background mock generation immediately
        ...
        async def _mock_process_queue(self, payload_str):
            try:
                ...
                import logging
                logging.getLogger("mock_queue").error(f"Error in local test queue mock: {e}")
...
# FSM states for website account linking flow
class LinkAccountStates(StatesGroup):
    waiting_for_hash = State()  # Awaiting hashed binding secret from the site

async def send_profile_screen(message_or_query: Any, db_user: Any, _: Callable[[str], str]) -> None:
    """Helper method to construct and display the user profile card."""
    # Fetch aggregated expenditure and purchases stats
    ...
    if isinstance(message_or_query, CallbackQuery):
        try:
            # Edit active photo caption on the menu banner if possible
            ...
            try:
                # Fallback to editing standard text message
                ...
                # Ultimate fallback: reply with new message
        ...
        # Replying to user's text command /profile
...
async def profile_callback(callback: CallbackQuery, db_user: Any, _: Callable[[str], str]):
    """Handles profile menu callback button clicks."""
...
async def toggle_language_callback(callback: CallbackQuery, db_user: Any, _: Callable[[str], str]):
    """
    Fallback interface language toggler (kept for backward compatibility).
    Switches locale (RU <-> EN).
    """
    new_lang = "ru" if db_user.language == "en" else "en"
    await update_user_language(db_user.telegram_id, new_lang)
    
    # Instantiate new localized translator and sync cached user model language immediately
...
async def my_purchases_callback(callback: CallbackQuery, db_user: Any, _: Callable[[str], str]):
    """Renders user's last 10 purchase logs with full history export utility buttons."""
...
async def my_deposits_callback(callback: CallbackQuery, db_user: Any, _: Callable[[str], str]):
    """Renders user's last 10 payment deposit logs."""
    ...
    if not txs:
        ...
        for tx in txs:
            ...
            # Render amounts and payment network details
...
async def linked_status_alert(callback: CallbackQuery, db_user: Any, _: Callable[[str], str]):
    """Quick modal pop-up advising user that site linkage is already established."""
    msg = _("link_success").format(login=db_user.site_login)
    await callback.answer(text=msg, show_alert=True)

# ----------------- FSM ACCOUNT BINDING / LINKING FLOW -----------------
...
async def start_account_linking(event: Any, db_user: Any, _: Callable[[str], str], state: FSMContext):
    """
    Initializes the marketplace profile binding sequence.
    Performs critical deposit validation checks before allowing balance merging.
    """
    # CRITICAL DOUBLE-SPEND / REVENUE MERGING SAFETY CHECK:
    # If the bot user already possesses active purchases or a balance exceeding $1.00,
    # auto-linking is strictly prohibited to prevent sybil balance exploits.
    # The user must contact support for a manual audit trail shift.
    ...
    async with async_session() as session:
        # Acquire row-level lock on user within database transaction
        locked_user = await get_user_by_id_for_update(db_user.telegram_id, session)
        if not locked_user:
            if isinstance(event, CallbackQuery):
                await event.answer("User profile not found.")
            return
            
        db_user.balance = locked_user.balance

        # Compile total user purchase count
    ...
    # If checks are passed, toggle FSM to waiting for hash input state
...
async def process_hash_input(message: Message, db_user: Any, _: Callable[[str], str], state: FSMContext):
    """Processes secret hash sequence token submitted by the user."""
    hash_password = message.text.strip()
    tg_id = message.from_user.id
    
    # Trigger typing state while performing network request calls
    ...
    # PHASE 1: Quick pre-transactional sanity check on balance
    async with async_session() as session:
    ...
    # PHASE 2: Execute network check outside DB transactions to prevent connection pool starvation on latency timeouts
    site_login = await api_client.check_hash(tg_id, hash_password)
    
    if site_login:
        # PHASE 3: Re-acquire row-level lock to guard against deposits processed during Phase 2, then commit
        async with async_session() as session:
            locked_user = await get_user_by_id_for_update(tg_id, session)
            
            # Double-check purchases and balance in final atomic block
            count_res = await session.execute(
                select(func.count(Purchase.id)).where(Purchase.telegram_id == tg_id)
            )
            p_count = count_res.scalar() or 0
            ...
            # Perform atomic profile linkage updates
            locked_user.site_login = site_login
            db_user.site_login = site_login  # Sync memory cache
        ...
        # Return user back to updated profile screen
        await send_profile_screen(message, db_user, _)
    else:
        # Invalid hash key
...
# ----------------- BACKGROUND PURCHASES HISTORY EXPORT -----------------
...
async def export_all_callback(callback: CallbackQuery, db_user: Any, _: Callable[[str], str]):
    """
    Handles background generation export task queue placement.
    Includes spam-protection rate limits (maximum 2 exports per 24 hours per user).
    Pushes generation metadata to Redis queue for sequential non-blocking processing.
    """
    tg_id = db_user.telegram_id
    
    # 1. Rate Limit Check: enforce a maximum of 2 requests every 24 hours
    from datetime import datetime
    ...
    if current_count == 1:
        await redis_client.expire(limit_key, 86400)  # Expire limiter key after 24 hours
        
    if current_count > 2:
        await callback.answer(text=_("export_limit_exceeded"), show_alert=True)
        return
        
    # 2. Check if user profile possesses purchases history to export
    ...
    # 3. Assemble background generation queue task payload
    task_payload = {
        "telegram_id": tg_id,
    ...
    # Notify client of successful queue placement
